...
def realization_process (itr, cfg, flow, output_dir):
    ...
    ####################################################################
    ptcl_density_data = torch.zeros((batch_size, n_steps+1+hist_len, ncells))
    ptcl_flux_data = torch.zeros((batch_size, n_steps+hist_len, ncells))
    ...
    for i_batch in range(batch_size):
        ...
        for i_step in range(n_steps+hist_len):
            ...
            #### ML model data is the same as particle solver for first hist_len
            if i_step < hist_len:
                mdl_density_data[i_batch, i_step+1, :] = density[:]
                mdl_flux_data[i_batch, i_step,:] = flux_ptcl[:]
    ####### ML model based time stepping #############################
    for i_step in range(hist_len, n_steps+hist_len):
        ###### ML model prediction ################################
        old_density = torch.narrow(mdl_density_data, 1, i_step-hist_len,
                                   hist_len+1).clone()
        ...
        ### Current shape (B, 1, ncells)
        ### Need to change the shape of old_density_r
        old_density_r = torch.swapdims(old_density_r, -2, -1) # (B, ncells, 1)

        N_left_r = torch.zeros_like(old_density_r)
        N_left_r[:,1:,0] = old_density_r[:, :-1, 0]
        N_left_r[:,0,0] = old_density_r[:, -1, 0]
        N_right_r = torch.zeros_like(old_density_r)
        N_right_r[:,:,0] = old_density_r[:, :, 0]
        ##### Need to reshape both N_left_r and N_right_r
        N_left_r  = torch.reshape(N_left_r, (-1, 1, 1))
        N_right_r = torch.reshape(N_right_r, (-1, 1, 1))
        ...
        old_density_int = torch.where(prob_tensr < od_ceil-old_density,
                                      od_floor, od_ceil)
        # Similar for Flux
        old_flux = torch.narrow(mdl_flux_data, 1, i_step-hist_len,
                                hist_len).clone()
    ...
    ptcl_density_data = torch.narrow(ptcl_density_data.clone(), 1,
                                     hist_len, n_steps+1)
    ptcl_flux_data = torch.narrow(ptcl_flux_data.clone(), 1,
                                     hist_len, n_steps)

    mdl_density_data = torch.narrow(mdl_density_data.clone(), 1,
                                     hist_len, n_steps+1)
    mdl_flux_data = torch.narrow(mdl_flux_data.clone(), 1,
                                     hist_len, n_steps)
