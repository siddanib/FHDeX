...
Assumes flux_data has a shape    (batch_size, n_time_steps-1, ncells)
"""
@torch.no_grad()
...
ONLY designed for periodic boundaries for now.
THIS ONLY PROVIDES INCREMENT FOR EACH CELL
Expected model_ouputs shape:  (Batch*n_cells, 1)
"""
@torch.no_grad()
def convert_model_outputs_to_batched_system_data (model_outputs, ncells):
    model_outputs = torch.reshape(model_outputs,(-1, ncells, 1))
    flux_left = torch.zeros_like(model_outputs)
    flux_right = torch.zeros_like(flux_left)
