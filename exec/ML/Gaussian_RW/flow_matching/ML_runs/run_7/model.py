...
class Hierarchical_Model (Flow_Base):
    def __init__ (self, history_length, half_window, output_size,
                 n_layers, layer_width, act_func = torch.nn.ReLU(),
                 residual_con=True, time_embedding=128):
        super().__init__()
        ...
        for ilev in range(history_length+1):
            self.hierarchy_levels.append(
                    Flow_DeepONet((ilev+1)*half_window*2,
                                  output_size, n_layers, layer_width,
                                  act_func, residual_con, time_embedding))
    ...
    def forward (self, x_t, c, t):
        hlf_wnd = self.half_window
        # The condition to be passed
        cond = torch.narrow(c, -1, -2*hlf_wnd, 2*hlf_wnd)
        x = self.hierarchy_levels[0](x_t,cond,t)
        for ilev in range(1, self.max_level+1):
            cond = torch.narrow(c, -1, -2*hlf_wnd*(ilev+1),
                                2*hlf_wnd*(ilev+1))
            x += self.hierarchy_levels[ilev](x_t,cond,t)

        # This is the reflection of the input
        cond_flip = torch.narrow(c, -1, -2*hlf_wnd, 2*hlf_wnd).clone()
        cond_flip = torch.flip(cond_flip,dims=(-1,))
        # Note the -1 sign for x_t
        x_flip = self.hierarchy_levels[0](-x_t,cond_flip,t)
        for ilev in range(1, self.max_level+1):
            cond_flip = torch.narrow(c, -1, -2*hlf_wnd*(ilev+1),
                                2*hlf_wnd*(ilev+1)).clone()
            cond_flip = torch.flip(cond_flip,dims=(-1,))
            x_flip += self.hierarchy_levels[ilev](-x_t,cond_flip,t)
...
if __name__ == "__main__":
    ...
    hist_len = 0
    half_window = 2
    mdl = Hierarchical_Model(hist_len, half_window,
                             1, 2, 64)
    #levels_list = [0,1, 2]
    levels_list = []
    mdl.train_levels(levels_list)
    x = torch.randn(1,2*half_window*(hist_len+1))
    t = torch.tensor([[0.1]])
    mdl.max_level = hist_len
    #for _ in range(20):
    #    print(mdl(t, x, t))
    #    print(mdl.sample(t,x))
    #mdl.compile()
