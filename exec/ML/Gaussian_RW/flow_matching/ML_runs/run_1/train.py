...
def fhd_model_run (cfg):
    ...
    if cfg.model.train:
        ...
        for epch in range(max_epoch):
            # Train only higher level in the last 15 epochs
            if epch > max_epoch - 15:
                flow.max_level = 1
                flow.train_levels([1,])
            ...
            for _ in range(int(n_iter_per_epoch)+1):
                ...
                input_batch, output_batch = get_particle_data(N_cell_batch, history_length,
                                                              dx, dt, left_boundary,
                                                              right_boundary)
