...
class Flow_Transformer(Flow_Base):
    ...
    def forward_single(self, tgt, src_N, src_F, t):
        ...
        # Add type embedding
        src_N_emb += self.type_embed(self.type_N.to(src_N.device))
        src_F_emb = self.input_F_proj(src_F) # (B, S-1, d_model)
        # Adding position embedding
        src_F_emb = self.pos_enc(src_F)
        # Add type embedding
        src_F_emb += self.type_embed(self.type_F.to(src_F.device))
        ## Concatenate along sequence dimension
        src_emb = torch.cat([src_N_emb, src_F_emb], dim=-2)
        memory = self.encoder(src_emb)       # (B, 2S-1, d_model)
        # Decoder
