...
def realization_process (itr, cfg, flow, output_dir):
    ...
    mdl_flux_data = torch.zeros((n_steps, ncells))

    for i_step in range(n_steps):
        initial_pos, density, flux_p = random_walk_v2(ncells, nmoves, dt,
                                             initial_pos.clone(),
                                             left_boundary, right_boundary,
                                             len_system = len_system)

        ptcl_density_data[i_step+1, :] = density[:]
        ptcl_flux_data[i_step,:] = flux_p[:]
        ###### ML model prediction ################################
        ...
        flux_dk, _ = get_dk_flux(old_density.clone(), ncells, dx, dt,
                                 left_boundary, right_boundary, nmoves)
        ### Create x_0
        x_0 = torch.zeros(1, 1, ncells)
        x_0[0,0,:] = flux_dk[:]
        x_0 /= N_scale
