...
class Flow_Transformer(Flow_Base):
    def __init__(self, input_N_dim=5, input_F_dim=1, d_model=64,
        ...
        self.target_proj  = nn.Linear(1, self.d2_model)  # forecast values map to d_model

        # Positional encoding 
        self.pos_enc = PositionalEncoding(d_model, max_len=max_len)

        # Time Encoding
        self.time_enc = TimeEncoding(d_embed=d_embed, d_model=self.d2_model,
        ...
        self.decoder_layer = nn.TransformerDecoderLayer(self.d2_model, nhead,
                                                        dim_feedforward,
                                                        dropout=dropout,
                                                        batch_first=True)
        ...
        self.out = nn.Linear(self.d2_model, 1)
    ...
    def forward_single(self, tgt, src_N, src_F, t):
        ...
        # Encoder for N
        memory_N = self.encoder_N(src_N_emb).narrow(-2,-1,1)
        ...
        memory = torch.cat([memory_N, memory_F], dim=-1)
        # Decoder
        tgt_emb = self.target_proj(tgt)      # (B, L, 2d_model)
        # Adding time embedding
        tgt_emb += self.time_enc(t)
        dec_out = self.decoder(tgt_emb, memory) # (B, L, 2d_model)
...
if __name__ == "__main__":
    ...
    x_F = torch.randn(batch_sz, seq_len,input_F_dim)
    ...
    #y = mdl(x_t, x_N, x_F, t)
    #print(y)
    #y_flip = mdl(-x_t, x_N.flip(dims=[-1,]),
    #             -x_F, t)
    #print(y_flip)
